...
- ✅ **D3 合规评估**：立即评估（M1 开发启动前完成，比原建议的 M3 前更前置）——**需用户安排专业人士，评估范围为 decisions.md D3**
- ✅ **D4 占位资源**：客服邮箱与 FAQ 反馈入口暂用占位符，M3 上线前统一替换
- ⏳ **D5 M1–M3 开发会话成本回填**：维持原计划（¥6–37 当量粗估），开发完成后回填

**决策后仍挂起的行动项**：A1 合规评估安排（M1 前，用户）｜A2 忘记密码文档增补（M2 规划时）｜A3 北极星校准（上线后）｜A4 占位符替换（M3 前）｜A5 成本回填（开发后）
