...
```
*/
type Not$1<A extends boolean> = A extends true ? false : A extends false ? true : never;
/**
...
@category Numeric
*/
type IsInteger<T> = T extends bigint ? true : T extends number ? number extends T ? false : T extends PositiveInfinity | NegativeInfinity ? false : Not$1<IsFloat<T>> : false;
//#endregion
...
type _SomeExtend<TArray extends UnknownArray, Type, Options extends Required<SomeExtendOptions>> = IfNotAnyOrNever<TArray, {
  ifNot: TArray extends readonly [infer First, ...infer Rest] ? IsNever<First> extends true ? Or<Or<IsNever<Type>, IsAny<Type>>, Not$1<Options['strictNever']>> extends true ?
...
type VariableLengthArraySliceHelper<Array_ extends readonly unknown[], Start extends number, End extends number> = And<Not$1<IsNegative<Start>>, IsEqual$2<End, never>> extends true ? ArraySplice<Array_, 0, Start> : And<And<Not$1<IsNegative<Start>>, Not$1<IsNegative<End>>>, IsEqual$2<GreaterThan<End, Start>, true>> extends true ? ArraySliceByPositiveIndex<Array_, Start, End> : [];
...
Or<Not$1<Options['strictLiteralChecks']>, And<IsStringLiteral<S>, IsStringLiteral<Delimiter>>> extends true ? S extends `${infer Head}${Delimiter}${infer Tail}` ? SplitHelper<Tail, Delimiter, Options, [...Accumulator, Head]> : Delimiter extends '' ? S extends '' ? Accumulator : [...Accumulator, S] : [...Accumulator, S] :
...
//#endregion
//#region src/internal/types/Boxed.d.ts
...
declare namespace Boxed {
  type Extract<T extends Boxed> = T[typeof VALUE];
}
//#endregion
//#region src/endsWith.d.ts
type EndsWith<T, Suffix extends string> = T & `${string}${Suffix}`;
type IsDisjointSuffix<T extends string, Suffix extends string> = [string] extends [Suffix] ? false : [string] extends [T] ? false : IsNever<EndsWith<T, Suffix>>;
type IsGuaranteedSuffix<T extends string, Suffix extends string> = [T] extends [EndsWithEvery<T, Suffix>] ? true : false;
type DisjointSuffixError<Suffix extends string> = RemedaTypeError<"endsWith", "This suffix doesn't match any of the inputs, the function will always return `false`", {
  type: string;
  metadata: Suffix;
}>;
type EndsWithEvery<T, Suffix extends string> = T & Boxed.Extract<UnionToIntersection<Suffix extends unknown ? Boxed<IsStringLiteral<Suffix> extends true ? `${string}${Suffix}` : never> : never>>;
type IsNarrowingUnsound$1<T, Suffix extends string> = IsEqual$2<IsEqual$2<Exclude<T, EndsWith<T, Suffix>>, Exclude<T, EndsWithEvery<T, Suffix>>>, false>;
/**
 ...
 * @hidden
 */
declare function endsWith<T extends string, Suffix extends string>(data: T, suffix: IsDisjointSuffix<T, Suffix> extends true ? DisjointSuffixError<Suffix> : IsGuaranteedSuffix<T, Suffix> extends true ? Suffix : never): true;
/**
 ...
 * @category String
 */
declare function endsWith<T extends string, Suffix extends string>(data: T, suffix: string extends Suffix ? never : IsDisjointSuffix<T, Suffix> extends true ? DisjointSuffixError<Suffix> : IsNarrowingUnsound$1<T, Suffix> extends true ? never : Suffix): data is EndsWith<T, Suffix>;
declare function endsWith<T extends string, Suffix extends string>(data: T, suffix: IsDisjointSuffix<T, Suffix> extends true ? DisjointSuffixError<Suffix> : Suffix): boolean;
/**
 * Determines whether a string ends with the provided suffix, and refines the
 * output type if possible.
 *
 ...
 * @category String
 */
declare function endsWith<T extends string, Suffix extends string>(suffix: IsDisjointSuffix<T, Suffix> extends true ? Suffix : never): (data: T & DisjointSuffixError<Suffix>) => boolean;
declare function endsWith<T extends string, Suffix extends string>(suffix: IsGuaranteedSuffix<T, Suffix> extends true ? Suffix : never): (data: T) => true;
declare function endsWith<T extends string, Suffix extends string>(suffix: IsNarrowingUnsound$1<T, Suffix> extends true ? Suffix : never): (data: T) => boolean;
declare function endsWith<Suffix extends string>(suffix: string extends Suffix ? never : Suffix): <T extends string>(data: T) => data is EndsWith<T, Suffix>;
...
//#endregion
//#region src/internal/types/Narrowed.d.ts
/**
 ...
 */
type Narrowed<T, Condition> = IsAny<T> extends true ? Condition : T extends Condition ? T : Condition extends T ? Condition : CanIntersect<T, Condition> extends true ? T & Condition : never;
/**
 ...
 */
type CanIntersect<T0, T1> = T0 extends Primitive ? T1 extends Primitive ? true : false : T1 extends Primitive ? false : IsObjectLike<T0> extends true ? IsObjectLike<T1> : false;
/**
 ...
 */
type IsObjectLike<T> = T extends object ? T extends readonly unknown[] ? false : T extends StrictFunction ? false : true : false;
//#endregion
//#region src/internal/types/Assignability.d.ts
/**
 ...
 * @see FindLast
 */
type Assignability<T, Condition, Cases extends Record<"full" | "partial" | "none", unknown>> = [T] extends [Condition] ? Cases["full"] : IsNever<Narrowed<T, Condition>> extends true ? Cases["none"] : Cases["partial"];
//#endregion
 ...
 */
type FilteredArray<T extends IterableContainer, Condition, IsNegated extends boolean = false> = T extends unknown ? [...FilteredFixedTuple<TupleParts<T>["required"], Condition, IsNegated>, ...PartialArray<FilteredFixedTuple<TupleParts<T>["optional"], Condition, IsNegated>>, ...CoercedArray<RefinedItem<TupleParts<T>["item"], Condition, IsNegated>>, ...FilteredFixedTuple<TupleParts<T>["suffix"], Condition, IsNegated>] : never;
type FilteredFixedTuple<T, Condition, IsNegated extends boolean> = T extends readonly [infer Head, ...infer Rest] ? [...FilteredItem<Head, Condition, IsNegated>, ...FilteredFixedTuple<Rest, Condition, IsNegated>] : [];
type FilteredItem<Item, Condition, IsNegated extends boolean> = Assignability<Item, Condition, {
  full: IsNegated extends true ? [] : [Item];
  none: IsNegated extends true ? [Item] : [];
  partial: [RefinedItem<Item, Condition, IsNegated>] | [];
}>;
type RefinedItem<Item, Condition, IsNegated extends boolean> = IsNegated extends true ? Exclude<Item, Condition> : Narrowed<Item, Condition>;
//#endregion
//#region src/internal/types/NonEmptyPrefix.d.ts
/**
 ...
 *   ): (data: T) => Mapped<T, U>;
 */
type NonEmptyPrefix<T extends IterableContainer> = T extends unknown ? HeadPrefixes<[...TupleParts<T>["required"], ...TupleParts<T>["optional"]], TupleParts<T>["item"]> | SuffixPrefixes<TupleParts<T>["required"], TupleParts<T>["item"], TupleParts<T>["suffix"]> : never;
type HeadPrefixes<Prefix, Item> = Prefix extends readonly [] ? IsNever<Item> extends true ? never : [Item, ...Item[]] : [...PartialTail<Prefix>, ...CoercedArray<Item>];
type SuffixPrefixes<Required extends unknown[], Item, Suffix> = Required extends readonly [] ? [Item, ...Item[], ...Prefixes<Suffix>] | PartialTail<Suffix> : [...Required, ...CoercedArray<Item>, ...Prefixes<Suffix>];
/**
 ...
 */
type PartialTail<T> = T extends readonly [infer Head, ...infer Tail] ? [Head, ...PartialArray<Tail>] : never;
/**
 ...
 */
type Prefixes<T> = T extends readonly [infer Head, ...infer Rest] ? [Head] | [Head, ...Prefixes<Rest>] : never;
//#endregion
//#region src/internal/types/LazyCallback.d.ts
/**
 * Helper type for lazy callbacks.
 *
 * @see NonEmptyPrefix
 */
type LazyCallback<T extends IterableContainer, R> = (value: T[number], index: number, data: Readonly<NonEmptyPrefix<T>>) => R;
/**
 ...
 * @see NonEmptyPrefix
 */
type LazyTypePredicate<T extends IterableContainer, S> = (value: T[number], index: number, data: Readonly<NonEmptyPrefix<T>>) => value is S;
//#endregion
//#region src/internal/types/WritableTuple.d.ts
/**
 * Drop-in replacement for type-fest's `Writable` type (for arrays/tuples) that
 * retains the input's shape.
 */
type WritableTuple<T extends IterableContainer> = T extends unknown ? [...TupleParts<T>["required"], ...PartialArray<TupleParts<T>["optional"]>, ...CoercedArray<TupleParts<T>["item"]>, ...TupleParts<T>["suffix"]] : never;
//#endregion
//#region src/internal/types/NonRefinedFilteredArray.d.ts
/**
 ...
 */
type NonRefinedFilteredArray<T extends IterableContainer, IsItemIncluded extends boolean> = boolean extends IsItemIncluded ? T[number][] : IsItemIncluded extends true ? WritableTuple<T> : [];
//#endregion
 ...
 * @category Array
 */
declare function filter<T extends IterableContainer, Condition>(predicate: LazyTypePredicate<T, Condition>): (data: T) => FilteredArray<T, Condition>;
declare function filter<T extends IterableContainer, IsItemIncluded extends boolean>(predicate: LazyCallback<T, IsItemIncluded>): (data: T) => NonRefinedFilteredArray<T, IsItemIncluded>;
//#endregion
//#region src/internal/types/First.d.ts
/**
 * The first element of `T`, or `undefined` when `T` might be empty.
 */
type First$1<T extends IterableContainer> = T extends unknown ? TupleParts<T>["required"] extends readonly [] ? TupleParts<T>["optional"][number] | TupleParts<T>["item"] | TupleParts<T>["suffix"][0] : TupleParts<T>["required"][0] : never;
//#endregion
//#region src/find.d.ts
type Found<T extends IterableContainer, Condition> = T extends unknown ? FoundInFixedTuple<TupleParts<T>["required"], Condition, Narrowed<TupleParts<T>["optional"][number], Condition> | Narrowed<TupleParts<T>["item"], Condition> | FoundInFixedTuple<TupleParts<T>["suffix"], Condition, undefined>> : never;
type FoundInFixedTuple<T, Condition, Fallback> = T extends readonly [infer Head, ...infer Rest] ? Assignability<Head, Condition, {
  full: Head;
  partial: Narrowed<Head, Condition> | FoundInFixedTuple<Rest, Condition, Fallback>;
  none: FoundInFixedTuple<Rest, Condition, Fallback>;
}> : Fallback;
type FoundNonRefined<T extends IterableContainer, IsItemIncluded extends boolean> = boolean extends IsItemIncluded ? T[number] | undefined : IsItemIncluded extends true ? First$1<T> : undefined;
/**
 ...
 * @category Array
 */
declare function find<T extends IterableContainer, Condition>(data: T, predicate: (value: T[number], index: number, data: T) => value is Condition): Found<T, Condition>;
declare function find<T extends IterableContainer, IsItemIncluded extends boolean>(data: T, predicate: (value: T[number], index: number, data: T) => IsItemIncluded): FoundNonRefined<T, IsItemIncluded>;
/**
 * Returns the first element in the provided array that satisfies the provided
 * testing function. If no values satisfy the testing function, `undefined` is
 * returned.
 ...
 * @category Array
 */
declare function find<T extends IterableContainer, Condition>(predicate: LazyTypePredicate<T, Condition>): (data: T) => Found<T, Condition>;
declare function find<T extends IterableContainer, IsItemIncluded extends boolean>(predicate: LazyCallback<T, IsItemIncluded>): (data: T) => FoundNonRefined<T, IsItemIncluded>;
//#endregion
...
type FoundLast<T extends IterableContainer, Condition> = T extends unknown ? FoundLastInFixedTuple<TupleParts<T>["suffix"], Condition, Narrowed<TupleParts<T>["item"], Condition> | Narrowed<TupleParts<T>["optional"][number], Condition> | FoundLastInFixedTuple<TupleParts<T>["required"], Condition, undefined>> : never;
type FoundLastInFixedTuple<T, Condition, Fallback> = T extends readonly [...infer Rest, infer Last] ? Assignability<Last, Condition, {
  full: Last;
  partial: Narrowed<Last, Condition> | FoundLastInFixedTuple<Rest, Condition, Fallback>;
  none: FoundLastInFixedTuple<Rest, Condition, Fallback>;
}> : Fallback;
type FoundLastNonRefined<T extends IterableContainer, IsItemIncluded extends boolean> = boolean extends IsItemIncluded ? T[number] | undefined : IsItemIncluded extends true ? LastArrayElement<T> : undefined;
/**
 ...
 * @category Array
 */
declare function findLast<T extends IterableContainer, Condition>(data: T, predicate: (value: T[number], index: number, data: T) => value is Condition): FoundLast<T, Condition>;
declare function findLast<T extends IterableContainer, IsItemIncluded extends boolean>(data: T, predicate: (value: T[number], index: number, data: T) => IsItemIncluded): FoundLastNonRefined<T, IsItemIncluded>;
/**
 * Iterates the array in reverse order and returns the value of the first
 * element that satisfies the provided testing function. If no elements satisfy
 * the testing function, undefined is returned.
 ...
 * @category Array
 */
declare function findLast<T extends IterableContainer, Condition>(predicate: (value: T[number], index: number, data: T) => value is Condition): (data: T) => FoundLast<T, Condition>;
declare function findLast<T extends IterableContainer, IsItemIncluded extends boolean>(predicate: (value: T[number], index: number, data: T) => IsItemIncluded): (data: T) => FoundLastNonRefined<T, IsItemIncluded>;
//#endregion
 ...
 * @category Array
 */
declare function flatMap<T, U>(callbackfn: LazyCallback<readonly T[], readonly U[] | U>): (data: readonly T[]) => U[];
//#endregion
 ...
 * @category Array
 */
declare function forEach<T extends IterableContainer>(callbackfn: LazyCallback<T, void>): (data: T) => Writable$1<T>;
//#endregion
...
//#endregion
//#region src/internal/types/Not.d.ts
...
declare function isNot<T, Result extends boolean>(predicate: (data: T) => Result): (data: T) => Not<Result>;
//#endregion
 ...
 * @category Array
 */
declare function map<T extends IterableContainer, U>(callbackfn: LazyCallback<T, U>): (data: T) => Mapped<T, U>;
//#endregion
...
type LazyFeedbackCallback<T extends IterableContainer, U> = (previousValue: U, currentValue: T[number], currentIndex: number, data: Readonly<NonEmptyPrefix<T>>) => U;
/**
 ...
 * @category Array
 */
declare function mapWithFeedback<T extends IterableContainer, U>(data: T, callbackfn: LazyFeedbackCallback<T, U>, initialValue: U): Mapped<T, U>;
/**
 * Applies a function on each element of the array, using the result of the
 * previous application, and returns an array of the successively computed
 * values.
 ...
 * @category Array
 */
declare function mapWithFeedback<T extends IterableContainer, U>(callbackfn: LazyFeedbackCallback<T, U>, initialValue: U): (data: T) => Mapped<T, U>;
//#endregion
 ...
 * @category Array
 */
declare function partition<T extends IterableContainer, Condition>(data: T, predicate: (value: T[number], index: number, data: T) => value is Condition): [FilteredArray<T, Condition>, FilteredArray<T, Condition, true>];
declare function partition<T extends IterableContainer, IsItemIncluded extends boolean>(data: T, predicate: (value: T[number], index: number, data: T) => IsItemIncluded): [NonRefinedFilteredArray<T, IsItemIncluded>, NonRefinedFilteredArray<T, Not<IsItemIncluded>>];
/**
 * Splits a collection into two groups, the first of which contains elements the
 * `predicate` type guard passes, and the second one containing the rest.
 *
 ...
 * @category Array
 */
declare function partition<T extends IterableContainer, Condition>(predicate: (value: T[number], index: number, data: T) => value is Condition): (data: T) => [FilteredArray<T, Condition>, FilteredArray<T, Condition, true>];
declare function partition<T extends IterableContainer, IsItemIncluded extends boolean>(predicate: (value: T[number], index: number, data: T) => IsItemIncluded): (data: T) => [NonRefinedFilteredArray<T, IsItemIncluded>, NonRefinedFilteredArray<T, Not<IsItemIncluded>>];
//#endregion
...
type IsDisjointPrefix<T extends string, Prefix extends string> = [string] extends [Prefix] ? false : [string] extends [T] ? false : IsNever<StartsWith<T, Prefix>>;
type IsGuaranteedPrefix<T extends string, Prefix extends string> = [T] extends [StartsWithEvery<T, Prefix>] ? true : false;
type DisjointPrefixError<Prefix extends string> = RemedaTypeError<"startsWith", "This prefix doesn't match any of the inputs, the function will always return `false`", {
  type: string;
  metadata: Prefix;
}>;
type StartsWithEvery<T, Prefix extends string> = T & Boxed.Extract<UnionToIntersection<Prefix extends unknown ? Boxed<IsStringLiteral<Prefix> extends true ? `${Prefix}${string}` : never> : never>>;
type IsNarrowingUnsound<T, Prefix extends string> = IsEqual$2<IsEqual$2<Exclude<T, StartsWith<T, Prefix>>, Exclude<T, StartsWithEvery<T, Prefix>>>, false>;
/**
 * **NOTE**: every possible value of `data` starts with every possible value of
 ...
 * @hidden
 */
declare function startsWith<T extends string, Prefix extends string>(data: T, prefix: IsDisjointPrefix<T, Prefix> extends true ? DisjointPrefixError<Prefix> : IsGuaranteedPrefix<T, Prefix> extends true ? Prefix : never): true;
/**
 ...
 * @category String
 */
declare function startsWith<T extends string, Prefix extends string>(data: T, prefix: string extends Prefix ? never : IsDisjointPrefix<T, Prefix> extends true ? DisjointPrefixError<Prefix> : IsNarrowingUnsound<T, Prefix> extends true ? never : Prefix): data is StartsWith<T, Prefix>;
declare function startsWith<T extends string, Prefix extends string>(data: T, prefix: IsDisjointPrefix<T, Prefix> extends true ? DisjointPrefixError<Prefix> : Prefix): boolean;
/**
 * Determines whether a string begins with the provided prefix, and refines the
 * output type if possible.
 *
 ...
 * @category String
 */
declare function startsWith<T extends string, Prefix extends string>(prefix: IsDisjointPrefix<T, Prefix> extends true ? Prefix : never): (data: T & DisjointPrefixError<Prefix>) => boolean;
declare function startsWith<T extends string, Prefix extends string>(prefix: IsGuaranteedPrefix<T, Prefix> extends true ? Prefix : never): (data: T) => true;
declare function startsWith<T extends string, Prefix extends string>(prefix: IsNarrowingUnsound<T, Prefix> extends true ? Prefix : never): (data: T) => boolean;
declare function startsWith<Prefix extends string>(prefix: string extends Prefix ? never : Prefix): <T extends string>(data: T) => data is StartsWith<T, Prefix>;
...
//#endregion
//#region src/internal/types/NarrowedArray.d.ts
/**
 ...
 * @see takeLastWhile
 */
type NarrowedArray<T extends IterableContainer, Condition> = T extends unknown ? Assignability<T[number], Condition, {
 ...
 * @category Array
 */
declare function takeLastWhile<T extends IterableContainer, Condition>(data: T, predicate: (item: T[number], index: number, data: T) => item is Condition): NarrowedArray<T, Condition>;
declare function takeLastWhile<T extends IterableContainer, IsItemIncluded extends boolean>(data: T, predicate: (item: T[number], index: number, data: T) => IsItemIncluded): NonRefinedFilteredArray<T, IsItemIncluded>;
/**
 * Returns elements from the end of the array until the predicate returns false.
 * The returned elements will be in the same order as in the original array.
 *
 ...
 * @category Array
 */
declare function takeLastWhile<T extends IterableContainer, Condition>(predicate: (item: T[number], index: number, data: T) => item is Condition): (data: T) => NarrowedArray<T, Condition>;
declare function takeLastWhile<T extends IterableContainer, IsItemIncluded extends boolean>(predicate: (item: T[number], index: number, data: T) => IsItemIncluded): (data: T) => NonRefinedFilteredArray<T, IsItemIncluded>;
//#endregion
 ...
 * @category Array
 */
declare function takeWhile<T extends IterableContainer, Condition>(data: T, predicate: (item: T[number], index: number, data: T) => item is Condition): NarrowedArray<T, Condition>;
declare function takeWhile<T extends IterableContainer, IsItemIncluded extends boolean>(data: T, predicate: (item: T[number], index: number, data: T) => IsItemIncluded): NonRefinedFilteredArray<T, IsItemIncluded>;
/**
 * Returns elements from the array until predicate returns false.
 *
 * @param predicate - A function to execute for each element in the array. It
 ...
 * @category Array
 */
declare function takeWhile<T extends IterableContainer, Condition>(predicate: (item: T[number], index: number, data: T) => item is Condition): (data: T) => NarrowedArray<T, Condition>;
declare function takeWhile<T extends IterableContainer, IsItemIncluded extends boolean>(predicate: (item: T[number], index: number, data: T) => IsItemIncluded): (data: T) => NonRefinedFilteredArray<T, IsItemIncluded>;
//#endregion
//#region src/tap.d.ts
/**
 ...
 * @category Array
 */
declare function uniqueBy<T extends IterableContainer>(data: T, keyFunction: LazyCallback<T, unknown>): Deduped<T>;
/**
 * Returns a new array containing only one copy of each element in the original
 * list transformed by a function. Elements are compared by reference using Set.
 *
 ...
 * @category Array
 */
declare function uniqueBy<T extends IterableContainer>(keyFunction: LazyCallback<T, unknown>): (data: T) => Deduped<T>;
//#endregion
...
type LazyZippingFunction<T1 extends IterableContainer = IterableContainer, T2 extends IterableContainer = IterableContainer, Value = unknown> = (first: T1[number], second: T2[number], index: number, data: readonly [first: Readonly<NonEmptyPrefix<T1>>, second: T2]) => Value;
/**
 ...
 * @category Array
 */
declare function zipWith<T1 extends IterableContainer, T2 extends IterableContainer, Value>(second: T2, fn: LazyZippingFunction<T1, T2, Value>): (first: T1) => Value[];
/**
 * Creates a new list from two supplied lists by calling the supplied function
 * with the same-positioned element from each list.
 *
